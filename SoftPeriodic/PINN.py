...
class PINNs(models.Model):
    ...
    def net_f(self, cp):
        ...
        # === Periodic Boundary: x = 0 and x = L ===
        mask_left = tf.squeeze(tf.abs(x) < 1e-5)
        mask_right = tf.squeeze(tf.abs(x - L) < 1e-5)

        Phi_left = tf.boolean_mask(Phi, mask_left)
        Phi_right = tf.boolean_mask(Phi, mask_right)

        Phi_x_left = tf.boolean_mask(Phi_x, mask_left)
        Phi_x_right = tf.boolean_mask(Phi_x, mask_right)

        Phi_y_left = tf.boolean_mask(Phi_y, mask_left)
        Phi_y_right = tf.boolean_mask(Phi_y, mask_right)

        # Periodic losses
        loss_periodic_phi = tf.reduce_mean(tf.square(Phi_left - Phi_right))
        loss_periodic_phix = tf.reduce_mean(tf.square(Phi_x_left - Phi_x_right))
        loss_periodic_phiy = tf.reduce_mean(tf.square(Phi_y_left - Phi_y_right))

        loss_periodic = loss_periodic_phi + loss_periodic_phix + loss_periodic_phiy
        ...
        return f1, loss_kbbc, loss_kfbc , f5, loss_periodic
    
    @tf.function
    def train_step(self, cp):
        print("train step is called")
        with tf.GradientTape() as tape:
            f1, f2, f4, f5, f6 = self.net_f(cp)
            loss_GE = f1
            loss_KBBC = f2
            loss_KFBC = f4
            loss_dummy = f5
            loss_periodic = f6
            loss = loss_GE + loss_KBBC + loss_KFBC + loss_dummy + loss_periodic
        ...
        l7 = tf.reduce_mean(loss_periodic)
        tf.print('loss:', l1)
