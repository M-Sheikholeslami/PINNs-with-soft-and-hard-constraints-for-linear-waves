...
class PINNs(models.Model):
    def __init__(self, model, optimizer, epochs, **kwargs):
        ...
        self.hist = []
        self.epoch = 0
        self.omega = tf.Variable(initial_value=1.0, trainable=True, dtype=tf.float32)
        self.sopt = lbfgs_op([self.omega]+self.trainable_variables)
    ...
    def net_f(self, cp):
        ...
        target_KFBC = A * self.omega * tf.sin(k * x_surf - self.omega * t_surf)
        loss_kfbc = tf.reduce_mean(tf.square(Phi_z_surf - target_KFBC))

        # === Free Surface: z = 0, enforce ∂Φ/∂t + gA cos(kx - ωt) = 0 ===
        Phi_t_surf = tf.boolean_mask(Phi_t, mask_surface)
        target_DFBC = -A * g * tf.cos(k * x_surf - self.omega * t_surf)
        loss_dfbc = tf.reduce_mean(tf.square(Phi_t_surf - target_DFBC))

        f1 = tf.abs(Phi_xx + Phi_yy)
        f5 = Phi - Phi
        f1 = tf.reduce_mean(tf.square(f1))
        f5 = tf.reduce_mean(tf.square(f5))      
        return f1, loss_kbbc, loss_dfbc, loss_kfbc , f5
    
    @tf.function
    def train_step(self, cp):
        print("train step is called")
        with tf.GradientTape() as tape:
            f1, f2, f3, f4, f5 = self.net_f(cp)
            loss_GE = f1
            loss_KBBC = f2
            loss_DFBC = f3
            loss_KFBC = f4
            loss_dummy = f5
            loss = loss_GE + loss_KBBC + loss_KFBC + loss_DFBC + loss_dummy
         
        trainable_vars = self.trainable_variables
        grads = tape.gradient(loss, [self.omega] + trainable_vars)
        l1 = tf.reduce_mean(loss)
        l2 = tf.reduce_mean(loss_GE)
        l3 = tf.reduce_mean(loss_KBBC)
        l4 = tf.reduce_mean(loss_KFBC)
        l5 = tf.reduce_mean(loss_DFBC)
        l6 = tf.reduce_mean(loss_dummy)
        tf.print('loss:', l1)
        return loss, grads, tf.stack([l1, l2, l3, l4, l5, l6]) #, l7    
    ...
    def fit(self, cp):
        ...
        def func(params_1d):
            ...
            omegaList.append(self.omega.numpy())
        ...
        for epoch in range(self.epochs):
            tf.print('epoch:', self.epoch)
            loss, grads, hist = self.train_step(cp)
            self.optimizer.apply_gradients(zip(grads, [self.omega] + self.trainable_variables))
            self.hist.append(hist.numpy())
            self.epoch += 1
            print("self.omega = ", [self.omega])
            omegaList.append(self.omega.numpy())
        ...
        return np.array(self.hist), omegaList
