...
class PINNs(models.Model):
    ...
    def net_f(self, cp):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            pred = self.model(X)
            pred_bottom = pred[:self.xn*self.tn]
            pred_h = tf.tile(pred_bottom, [self.yn, 1])       
            pred_y = tape.gradient(pred, y)
            pred_y_bottom = pred_y[:self.xn*self.tn]
            pred_y_h = tf.tile(pred_y_bottom, [self.yn, 1])
            Phi = (tf.exp(y)) * (pred - pred_h - pred_y_h)
            # Phi = pred
            Phi_x = tape.gradient(Phi, x)
            Phi_y = (tf.exp(y)) * (pred - pred_h - pred_y_h + pred_y) 
            pred_yy = tape.gradient(pred_y, y)
        Phi_xx = tape.gradient(Phi_x, x)
        Phi_yy = (tf.exp(y)) * (pred - pred_h - pred_y_h + 2* pred_y + pred_yy) 
    ...
    def train_step(self, cp):
        ...
        tf.print('loss:', l1)
        return loss, grads, tf.stack([l1, l2, l3, l4, l6]) 
    ...
    def predict(self, cp):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            X = tf.squeeze(X, axis=1)
            pred = self.model(X)
            pred_bottom = pred[:31*31]
            pred_h = tf.tile(pred_bottom, [31, 1])  
            pred_y = tape.gradient(pred, y)
            pred_y_bottom = pred_y[:31*31]
            pred_y_h = tf.tile(pred_y_bottom, [31, 1])
            Phi = (tf.exp(y)) * (pred - pred_h - pred_y_h)
            Phi_x = tape.gradient(Phi, x)
            Phi_y = (tf.exp(y)) * (pred - pred_h - pred_y_h + pred_y) 
